    ...
    const fetchPosts = async () => {
      try {
        const response = await fetch(
          `http://localhost:8070/api/reorders/reorder/${id}`
        );
        ...
        const reorder = json.reorder;
        if (reorder) {
          setSupplierName(reorder.name);
          setProductName(reorder.productName);
          setProductID(reorder.productID);
          setQuantity(reorder.quantity);
          // setCountry(json.country);
        } else {
          console.error("Unexpected data structure:", reorder);
        }
      } catch (error) {
  ...
  function Generate() {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .put(`http://localhost:8070/api/reorders/materialCost/${id}`, {
            month: selectedMonth,
